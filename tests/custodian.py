...
def transfers():
    ...
    token.transfer(a[2],100)
    check.equal(issuer.getInvestorCounts()[0][0],1,"Investor count is wrong")
    check.equal(cust1.balanceOf(token,id2),100,"Custodian balance is wrong")
    token.transfer(a[3],100)
    cust1.transferInternal(token, id2, id3, 50, False)
    check.equal(cust1.balanceOf(token,id2),50,"Custodian balance is wrong")
    check.equal(cust1.balanceOf(token,id3),50,"Custodian balance is wrong")
    check.true(cust1.isBeneficialOwner(issuer,id2),"Custodian beneficial owner is wrong")
    check.true(cust1.isBeneficialOwner(issuer,id3),"Custodian beneficial owner is wrong")
    check.equal(issuer.getInvestorCounts()[0][0],2,"Investor count is wrong")
    cust1.transfer(token,a[3],50,False)
    check.false(cust1.isBeneficialOwner(issuer,id3),"Custodian beneficial owner is wrong")
    check.equal(issuer.getInvestorCounts()[0][0],2,"Investor count is wrong")
...
def transfers3():
    ...
    check.false(cust1.isBeneficialOwner(issuer,id2),"Custodian beneficial owner is wrong")
    check.true(cust1.isBeneficialOwner(issuer,id4),"Custodian beneficial owner is wrong")
    check.equal(cust1.balanceOf(token,id4),100,"Custodian balance is wrong")
    check.equal(cust1.balanceOf(token,id2),0,"Custodian balance is wrong")
    check.equal(issuer.getInvestorCounts()[0][0],1,"Investor count is wrong")
...
def transfers4():
    ...
    check.equal(cust2.balanceOf(token,id3),100,"Custodian balance is wrong")
    check.equal(issuer.getInvestorCounts()[0][0],2,"Investor count is wrong")
